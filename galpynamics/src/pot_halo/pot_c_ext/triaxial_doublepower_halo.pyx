...
cdef double psi_triaxial_doublepower(double d0, double rs, double alpha, double beta, double m) nogil:
    ...
    return -2*d0*m*m*pow(m/rs,-alpha)*hyp2f1(2.-alpha, beta-alpha, 3.-alpha, -m/rs)/(alpha-2.) 
...
cdef double integrand_triaxial_doublepower(int nn, double *data) nogil:
    ...
    if m==0.: return 0 
    # Psi at m
    if (m<=mcut): psi=psi_triaxial_doublepower(d0, rs, alpha, beta, m)
    else: psi=psi_triaxial_doublepower(d0, rs, alpha, beta, mcut)
    # Psi at inf (mcut)
    psicut = psi_triaxial_doublepower(d0, rs, alpha, beta, mcut)
...
cdef double _potential_triaxial_doublepower(double x, double y, double z, double mcut, double d0, double rs, double alpha, double beta, double a, double b, double c, double toll):
    ...
    #Integral
    import galpynamics.src.pot_halo.pot_c_ext.triaxial_doublepower_halo as mod
    fintegrand=LowLevelCallable.from_cython(mod,'integrand_triaxial_doublepower')
    
    # For convenience, we integrate in the variable s = 1/sqrt(tau +1) 
    intpot=quad(fintegrand,0.,1.,args=(x,y,z,mcut,d0,rs,alpha,beta,a,b,c,toll),epsabs=toll,epsrel=toll)[0]
...
cdef double[:,:] _potential_triaxial_doublepower_array(double[:] x, double[:] y, double[:] z, int nlen, double mcut, double d0, double rs, double alpha, double beta, double a, double b, double c, double toll):
    ...
    for i in range(nlen):
        ...
        ret[i,1]=y[i]
        ret[i,2]=z[i]
        ret[i,3]= _potential_triaxial_doublepower(x[i],y[i],z[i],mcut,d0,rs,alpha,beta,a,b,c,toll)
    ...
    for k in range(nlenz):
        for j in range(nleny):
            for  i in range(nlenx):
                ...
                ret[l,1]=y[j]
                ret[l,2]=z[k]                
                ret[l,3]=_potential_triaxial_doublepower(x[i],y[j],z[k],mcut,d0,rs,alpha,beta,a,b,c,toll) 
                l+=1
...
cpdef potential_triaxial_doublepower(x, y, z, d0, rs, alpha, beta, a, b, c, mcut, toll=1e-4, grid=False):

    if isinstance(x, (float,int)) and isinstance(y,(float,int)) and isinstance(z, (float,int)):
        return np.array(_potential_triaxial_doublepower(x=x,y=y,z=z,mcut=mcut,d0=d0,rs=rs,alpha=alpha,beta=beta,a=a,b=b,c=c,toll=toll))
    else:
        if grid:
            x=np.array(x,dtype=np.dtype("d"))
            y=np.array(y,dtype=np.dtype("d"))
            z=np.array(z,dtype=np.dtype("d"))
            return np.array(_potential_triaxial_doublepower_grid(x=x,y=y,z=z,nlenx=len(x),nleny=len(y),nlenz=len(z),mcut=mcut,d0=d0,rs=rs,alpha=alpha,beta=beta,a=a,b=b,c=c,toll=toll))
        elif len(x)==len(y)==len(z):
            nlen=len(x)
            x=np.array(x,dtype=np.dtype("d"))
            y=np.array(y,dtype=np.dtype("d"))
            z=np.array(z,dtype=np.dtype("d"))
            return np.array(_potential_triaxial_doublepower_array(x=x,y=y,z=z,nlen=len(x),mcut=mcut,d0=d0,rs=rs,alpha=alpha,beta=beta,a=a,b=b,c=c,toll=toll))
        else:
...
cdef double _vcirc_triaxial_doublepower(double R, double d0, double rs, double rb, double alpha, double e, double toll):
    """
    ...
    #Integ
    import galpynamics.src.pot_halo.pot_c_ext.triaxial_doublepower_halo as mod
    fintegrand=LowLevelCallable.from_cython(mod,'vcirc_integrand_triaxial_doublepower')
...
cdef double[:,:] _vcirc_triaxial_doublepower_array(double[:] R, int nlen, double d0, double rs, double rb, double alpha, double e, double toll):
    """
    Calculate Vcirc on a single point on the plane
    :param R: radii array (kpc)
    :param d0: Central density (Msol/kpc^3)
...
cpdef vcirc_triaxial_doublepower(R, d0, rs, rb, alpha, e, toll=1e-4):
    ...
    if isinstance(R, float) or isinstance(R, int):
        if R==0: ret=0
        else: ret= _vcirc_triaxial_doublepower(R,d0,rs,rb,alpha,e,toll)
    else:
        ret=_vcirc_triaxial_doublepower_array(R,len(R),d0,rs,rb,alpha,e,toll)
