...
class halo(object):
    ...
    def potential(self,R,Z=0,grid=False,toll=1e-4,mcut=None, nproc=1,output='1D'):
        ...
        if isinstance(Z,(int,float)): Z=np.array((Z,))
...
class NFW_halo(halo):
    ...
    def cosmo(cls, c, V200, H=67, e=0, mcut=100):
        """ Initialize NFW given concentration c and V200
...
class alfabeta_halo(halo):
    ...
    def __init__(self,d0,rs,alfa,beta,e=0,mcut=100):
        ...
        if alfa>=2:
            raise ValueError('alfa must be <2')
...
class powercut_halo(halo):
 
    def __init__(self,d0,rc,rb,alpha,e=0,mcut=100):
        """
        Power law with cut (e.g., Binney&Tremaine2008, eq. 2.207a)
        d = d0*(m/rs)**(-alpha)*exp(-(m/rb)**2)
         
        where m**2 = R**2 + z**2/(1-e**2))
 
        :param d0:  Central density in Msun/kpc^3
        :param rs:  Scale radius in kpc
        :param rc:  Cut radius in kpc
        :param e:  eccentricity (sqrt(1-b^2/a^2))
        :param alpha: index of power law
        :param beta: index of exponential cut
        :param mcut: elliptical radius where dens(m>mcut)=0
        """
 
        self.alpha = alpha
        self.rb = rb
        super(powercut_halo,self).__init__(d0=d0,rc=rc,e=e,mcut=mcut)
        self.name='Power law with cut halo'
 
 
    def _dens(self, R, Z=0):
        """ Return density at cylindrical (R,Z) """
        q2 = 1 - self.e*self.e
        m  = np.sqrt(R*R + Z*Z/q2)
        d  = self.d0*(m/self.rc)**(-alpha)*exp(-(m/self.rb)**2)
        return d
    
    def _potential_serial(self, R, Z, grid=False, toll=1e-4, mcut=None):
        """Calculate the potential in R and Z using a serial code """        
        self.set_toll(toll)
        return potential_powercut(R,Z,self.d0,self.rc,self.rb,self.alpha,self.e,mcut,self.toll,grid)
    ...
    def _potential_parallel(self, R, Z, grid=False, toll=1e-4, mcut=None, nproc=2):
        ...
        self.set_toll(toll)
        pardo=ParDo(nproc=nproc)
        pardo.set_func(potential_powercut)
        if len(R)!=len(Z) or grid==True:
